...
class CoaxWellBores(WellBores.WellBores, AdvGeoPHIRESUtils.AdvGeoPHIRESUtils):
    """
    CoaxWellBores Child class of WellBores; it is the same, but has advanced coaxial closed-loop functionality
    ...
    def __init__(self, model:AdvModel):
        ...
        self.ITDiameter = self.ParameterDict[self.ITDiameter.Name] = floatParameter(""Insulated Tube Diameter", value = 6.8, DefaultValue=6.8, Min=1, Max=30, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.INCHES, CurrentUnits = LengthUnit.INCHES, Required=True, ErrMessage = "assume default insulated tube diameter (6.8 inch)", ToolTipText="Inner diameter of insulated tube")
    ...
    def Calculate(self, model:AdvModel)-> None:
        ...
        #note that we don't call the calculate method on the superclass because we are calculating everything from scratch
        ...
        #special case: production and injection well diameters are input as inches and call calculations assume meters! Check and change if needed, assuming anything > 2 muyst be talkning about inches
        if self.injwelldiam.value > 2.0: self.injwelldiam.value = self.injwelldiam.value * 0.0254
        if self.prodwelldiam.value > 2.0: self.prodwelldiam.value = self.prodwelldiam.value * 0.0254
        ...
        if self.impedancemodelused.value: #assumed everything stays liquid throughout
            ...
            self.DPInjWell.value = f1*(self.rhowaterinj*vinj**2/2)*(model.reserv.depth.value/self.injwelldiam.value)/1E3      #/1E3 to convert from Pa to kPa
            ...
            self.DPReserv.value = self.impedance.value*self.nprod.value*self.prodwellflowrate.value*1000./rhowaterreservoir
    
            #production well pressure drop [kPa]
            self.DPProdWell.value = f3*(rhowaterprod*vprod**2/2.)*(model.reserv.depth.value/self.prodwelldiam.value)/1E3     #/1E3 to convert from Pa to kPa
    
            #buoyancy pressure drop [kPa]
            self.DPBouyancy.value = (rhowaterprod-self.rhowaterinj)*model.reserv.depth.value*9.81/1E3 # /1E3 to convert from Pa to kPa
    
            #overall pressure drop
            self.DPOverall.value = self.DPInjWell.value + self.DPReserv.value + self.DPProdWell.value + self.DPBouyancy.value

            #calculate pumping power [MWe] (approximate)
            self.PumpingPower.value = self.DPOverall.value*self.nprod.value*self.prodwellflowrate.value*(1+model.reserv.waterloss.value)/self.rhowaterinj/model.surfaceplant.pumpeff.value/1E3   
            ...
            if self.productionwellpumping.value:
                ...
                self.DPProdWell.value = self.Pprodwellhead.value - (self.Phydrostaticcalc.value - self.prodwellflowrate.value/PIkPa - rhowaterprod*9.81*model.reserv.depth.value/1E3 - f3*(rhowaterprod*vprod**2/2.)*(model.reserv.depth.value/self.prodwelldiam.value)/1E3)
                self.PumpingPowerProd.value = self.DPProdWell.value*self.nprod.value*self.prodwellflowrate.value/rhowaterprod/model.surfaceplant.pumpeff.value/1E3 #[MWe] total pumping power for production wells
            ...
            self.DPInjWell.value = self.Pinjwellhead-model.surfaceplant.Pplantoutlet.value
            #wellbores.DPInjWell.value = [0 if x<0 else x for x in DPInjWell] #set negative values to 0
            self.PumpingPowerInj.value = self.DPInjWell.value*self.nprod.value*self.prodwellflowrate.value*(1+model.reserv.waterloss.value)/self.rhowaterinj/model.surfaceplant.pumpeff.value/1E3 #[MWe] total pumping power for injection wells

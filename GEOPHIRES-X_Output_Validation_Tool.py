...
def main():
    ...
    with open(args.Input_file, mode = "r", encoding='UTF-8') as f1:
        ...
        #This is where we start the loop of however many GEOPHIRES-X runs
            while True:
                ...
                sprocess = subprocess.Popen(["python", python_code_to_run, input_control_file, output_result_file]) # allow stdout to be printed to screen:  , stdout=subprocess.DEVNULL)
                sprocess.wait()
                ...
                with open(output_result_file, mode = "r", encoding='UTF-8') as f3: output_lines = f3.readlines()
                with open(Precomputed_results_file, mode = "r") as f4: precomputed_lines = f4.readlines()
                ...
                for search_string in search_strings:
                    ...
                    pair3 = search_string.split("|")    #If pair has a "|", it must have 2 different spellings for search string
                    if len(pair3) > 1:
                        search_string = pair3[0]
                        pc_search_string = pair3[1]
                    else: pc_search_string = search_string
                    ...
                    if len(pair) == 1:
                        ...
                        for output_line in output_lines:
                            ...
                            if search_string in output_line:
                                ...
                                if len(pair) == 1: pair = output_line.split("=")   #If there isn't a ":" in search string, then we have an old-style outfile file, which uses "="
                        ...
                        for precomputed_line in precomputed_lines:
                            if pc_search_string in precomputed_line:
                                ...
                                if len(pair) == 1: pair = precomputed_line.split("=")   #If there isn't a ":" in search string, then we have an old-style outfile file, which uses "="
                        ...
                        if result_value is not None and precomputed_value is not None:
                            ...
                            if not percent_difference_calc:
                                if result_value != precomputed_value: f2.write("for " + search_string + ", values DO NOT MATCH! result value = " + str(result_value) + "; precomputed value = " + str(precomputed_value) + "\n")
                                else: pass #don't report anything if they match
                            else:
                                pd = 100.0 * (result_value-precomputed_value) / ((result_value+precomputed_value)/2.0)
                                if pd > percent_difference: f2.write("for " + search_string + ", values EXCEED A PERCENT DIFFERENCE OF " + str(percent_difference) + "%! result value = " + str(result_value) + "; precomputed value = " + str(precomputed_value) + "\n")
                                else: pass #don't report anything if they match
                        else:
    ...
    with open(output_file,'r', encoding='UTF-8') as f: content = f.readlines()    #store all output in one long list

...
class CLGSWellBores(WellBores.WellBores, AdvGeoPHIRESUtils.AdvGeoPHIRESUtils):
    ...
    def __init__(self, model:AdvModel):
        ...
        self.Drilling_length = self.OutputParameterDict[self.Drilling_length.Name] = OutputParameter(Name = "Drilling length", value=0.0, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.METERS, CurrentUnits = LengthUnit.METERS)
        self.CLGSProducedTemperature = self.OutputParameterDict[self.ProducedTemperature.Name] = OutputParameter(Name = "CLGS Produced Temperature", value=[0.0], UnitType = Units.TEMPERATURE, PreferredUnits = TemperatureUnit.CELCIUS, CurrentUnits = TemperatureUnit.CELCIUS)
        self.CLGSProducedTemperature.value = [0.0]*model.surfaceplant.plantlifetime.value #intialize the array
        self.CLGSPressureDrop = self.OutputParameterDict[self.CLGSPressureDrop.Name] = OutputParameter(Name = "CLGS Pressure Drop", value=[0.0], UnitType = Units.PRESSURE, PreferredUnits = PressureUnit.KPASCAL, CurrentUnits = PressureUnit.KPASCAL)
...
#        model.reserv.gradient.PreferredUnits = TemperatureGradientUnit.DEGREESCPERM
    ...
    #code from Koenraad
    def calculatedrillinglength(self) -> float:
        if self.Configuration.value == Configuration.ULOOP: return (self.Hor_length.value + 2*self.Depth)  #Total drilling depth of both wells and lateral in U-loop [m]
        else: return (self.Hor_length.value + self.Depth)  #Total drilling depth of well and lateral in co-axial case [m]
    ...
    def Calculate(self, model:AdvModel) -> None:
        ...
        self.Drilling_length.value = self.calculatedrillinglength()

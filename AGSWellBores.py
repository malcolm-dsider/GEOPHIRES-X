...
#ref Beckers, Koenraad, et al. Tabulated Database of Closed-Loop Geothermal Systems Performance for Cloud-Based Technical and Economic Modeling of Heat Production and Electricity Generation. No. NREL/CP-5700-84979. National Renewable Energy Lab.(NREL), Golden, CO (United States), 2023.
import sys
import os
import math
import numpy as np
...
class AGSWellBores(WellBores.WellBores, AdvGeoPHIRESUtils.AdvGeoPHIRESUtils):
    """
    ...
    def __init__(self, model:AdvModel):
        ...
        self.Fluid = self.ParameterDict[self.Fluid.Name] = intParameter("Heat Transfer Fluid", value = WorkingFluid.WATER, DefaultValue=WorkingFluid.WATER, AllowableRange=[1,2], UnitType = Units.NONE, Required=True, ErrMessage = "assume default Heat transfer fluid is water (1)")
        ...
        self.Nonvertical_length = self.ParameterDict[self.Nonvertical_length.Name] = floatParameter("Total Nonvertical Length", value = 1000.0, DefaultValue=1000.0, Min=1000.0, Max=20000.0, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.METERS, CurrentUnits = LengthUnit.METERS, Required=True, ErrMessage = "assume default Total nonvertical length (1000 m)")
        ...
        # already have it:self.prodwelldiam/39.37        self.Diameter = self.ParameterDict[self.Diameter.Name] = floatParameter("Well Diameter", value = 0.4445, DefaultValue=0.4445, Min=0.2159, Max=0.4445, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.METERS, CurrentUnits = LengthUnit.METERS, Required=True, ErrMessage = "assume default Well diameter (.4445 m)")
...
#        self.l_pipe = self.ParameterDict[self.l_pipe.Name] = floatParameter("Nonvertical Wellbore Length", value = 5000.0, DefaultValue=5000.0, Min=0.01, Max = 100_000.0, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.METERS, CurrentUnits = LengthUnit.METERS, ErrMessage="assume default for Nonvertical Wellbore Length (5000.0 m)", ToolTipText="Nonvertical Wellbore Length")
        self.nonverticalwellborediameter = self.ParameterDict[self.nonverticalwellborediameter.Name] = floatParameter("Nonvertical Wellbore Diameter", value = 0.156, DefaultValue=0.156, Min=0.01, Max = 100.0, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.METERS, CurrentUnits = LengthUnit.METERS, ErrMessage="assume default for Nonvertical Wellbore Diameter (0.156 m)", ToolTipText="Nonvertical Wellbore Diameter")
        self.numnonverticalsections = self.ParameterDict[self.numnonverticalsections.Name] = intParameter("Number of Multilateral Sections", value = 1, DefaultValue=1, AllowableRange=list(range(0,101,1)), UnitType = Units.NONE, ErrMessage="assume default for Number of Nonvertical Wellbore Sections (1)", ToolTipText="Number of Nonvertical Wellbore Sections")
        ...
        self.NonverticalsCased = self.ParameterDict[self.NonverticalsCased.Name] = boolParameter("Multilaterals Cased", value=False, DefaultValue=False, Required=False, Provided=False, Valid=True, ErrMessage= "assume feault value (False)")
...
#        self.Tin = self.OutputParameterDict[self.Tin.Name] = OutputParameter(Name = "Injection Temperature", value=-999.0, UnitType = Units.TEMPERATURE, PreferredUnits = TemperatureUnit.CELCIUS, CurrentUnits = TemperatureUnit.CELCIUS)
#        self.Diameter = self.OutputParameterDict[self.Diameter.Name] = OutputParameter("Well Diameter", value = -999.0, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.METERS, CurrentUnits = LengthUnit.METERS)
 #       self.Gradient = self.OutputParameterDict[self.Gradient.Name] = OutputParameter("Gradient", value = -999.0, UnitType = Units.TEMP_GRADIENT, PreferredUnits = TemperatureGradientUnit.DEGREESCPERM, CurrentUnits = TemperatureGradientUnit.DEGREESCPERM)
#        self.Depth = self.OutputParameterDict[self.Depth.Name] = OutputParameter("Vertical Depth", value = -999.0, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.METERS, CurrentUnits = LengthUnit.METERS)
#        self.Flow = self.OutputParameterDict[self.Flow.Name] = OutputParameter("Total Mass Flow Rate", value = -999.0, UnitType = Units.FLOWRATE, PreferredUnits = FlowRateUnit.KGPERSEC, CurrentUnits = FlowRateUnit.KGPERSEC)
#        self.Drilling_length = self.OutputParameterDict[self.Drilling_length.Name] = OutputParameter(Name = "Drilling length", value=-999.0, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.METERS, CurrentUnits = LengthUnit.METERS)
#        self.AGSProducedTemperature = self.OutputParameterDict[self.ProducedTemperature.Name] = OutputParameter(Name = "AGS Produced Temperature", value=[0.0], UnitType = Units.TEMPERATURE, PreferredUnits = TemperatureUnit.CELCIUS, CurrentUnits = TemperatureUnit.CELCIUS)
#        self.AGSProducedTemperature.value = [0.0]*model.surfaceplant.plantlifetime.value #intialize the array
#        self.AGSPressureDrop = self.OutputParameterDict[self.AGSPressureDrop.Name] = OutputParameter(Name = "AGS Pressure Drop", value=[0.0], UnitType = Units.PRESSURE, PreferredUnits = PressureUnit.KPASCAL, CurrentUnits = PressureUnit.KPASCAL)
#        self.AGSPressureDrop.value = [0.0]*model.surfaceplant.plantlifetime.value #intialize the array
        self.Tini = 0.0

        #results are stored here and in the parent ProducedTemperature array
        self.NonverticalProducedTemperature = self.OutputParameterDict[self.ProducedTemperature.Name] = OutputParameter(Name = "Nonvertical Produced Temperature", value=[0.0], UnitType = Units.TEMPERATURE, PreferredUnits = TemperatureUnit.CELCIUS, CurrentUnits = TemperatureUnit.CELCIUS)
        self.NonverticalProducedTemperature.value = [0.0]*(model.surfaceplant.plantlifetime.value) #intialize the array
        self.NonverticalPressureDrop = self.OutputParameterDict[self.NonverticalPressureDrop.Name] = OutputParameter(Name = "Nonvertical Pressure Drop", value=[0.0], UnitType = Units.PRESSURE, PreferredUnits = PressureUnit.KPASCAL, CurrentUnits = PressureUnit.KPASCAL)
        self.NonverticalPressureDrop.value = [0.0]*model.surfaceplant.plantlifetime.value #intialize the array
            ...
            for item in self.ParameterDict.items():
                ...
                if key in model.InputParameters:
                    ...
                    if ParameterToModify.Name == "Heat Transfer Fluid":
                        if ParameterReadIn.sValue == str(1): self.Fluid.value = WorkingFluid.WATER
                        else: self.Fluid.value = WorkingFluid.SCO2
                    if ParameterToModify.Name == "Closed-loop Configuration":
                        if ParameterReadIn.sValue == str(1): self.Configuration.value = Configuration.ULOOP
                        else: self.Configuration.value = Configuration.COAXIAL
        else:
...
#        self.Depth.value = model.reserv.InputDepth.value * 1000.0     #convert to meters
#        self.Depth.CurrentUnits = LengthUnit.METERS
#        self.Flow.value = self.prodwellflowrate.value   #same units are GEOPHIRES
#        self.Diameter.value = self.prodwelldiam.value/39.37   #convert inches to meters
#        self.Diameter.CurrentUnits = LengthUnit.METERS
...
#        model.reserv.InputDepth.value = self.Depth.value
#        model.reserv.InputDepth.CurrentUnits = LengthUnit.METERS
#        self.prodwelldiam.value = self.Diameter.value
#        self.prodwelldiam.CurrentUnits = LengthUnit.METERS

        model.logger.info("complete "+ str(__class__) + ": " + sys._getframe().f_code.co_name)
    
    #code from Koenraad
    def calculatedrillinglengths(self, model) -> float:
        ...
        if self.Configuration.value == Configuration.ULOOP: 
            return ((self.numnonverticalsections.value * self.Nonvertical_length.value) + 2*model.reserv.InputDepth.value * 1000.0), 2*model.reserv.InputDepth.value * 1000.0, self.numnonverticalsections.value * self.Nonvertical_length.value    #Total drilling depth of both wells and laterals in U-loop [m]
        else:
            return (self.Nonvertical_length.value + model.reserv.InputDepth.value * 1000.0), model.reserv.InputDepth.value * 1000.0, self.Nonvertical_length.value  #Total drilling depth of well and lateral in co-axial case [m]
    ...
    def getTandP(self, model:AdvModel) -> None:
        ...
        model.logger.info("Init " + str(__class__) + ": " + sys._getframe().f_code.co_name)
                #code from Koenraad
        self.point = (self.prodwellflowrate.value, self.Nonvertical_length.value, model.reserv.InputDepth.value * 1000.0, model.reserv.gradient.value[0], self.prodwelldiam.value/39.37, self.Tinj.value + 273.15, self.krock)
    ...
    def verify(self, model:AdvModel) -> int:
        ...
        self.error = 0
#        if self.Flow.value < 5 or self.Flow.value > 100:
...
#            self.error = 1
        if self.Nonvertical_length.value < 1000 or self.Nonvertical_length.value > 20000:
            print("Error: CLGS model database imposes additional range restrictions: Nonvertical length must be between 1,000 and 20,000 m. Simulation terminated.")
            model.logger.fatal("Error: CLGS model database imposes additional range restrictions: Nonvertical length must be between 1,000 and 20,000 m. Simulation terminated.")
            self.error = 1
#        if self.Depth.value < 1000 or self.Depth.value > 5000:
...
#            self.error = 1
#        if self.Gradient.value < 0.03 or self.Gradient.value > 0.07:
...
#            self.error = 1
#        if self.Diameter.value < 0.2159 or self.Diameter.value > 0.4445:
...
#            self.error = 1
        if self.Tinj.value < 30.0 or self.Tinj.value > 60.0:
            print("Error: CLGS model database imposes additional range restrictions: Injection temperature must be between 30 and 60 C. Simulation terminated.")
            model.logger.fatal("Error: CLGS model database imposes additional range restrictions: Injection temperature must be between 30 and 60 C. Simulation terminated.")
            self.error = 1
    ...
    def laplace_solution(self, sp, model):
        ...
        Toutletl = (self.Tini - self.Tinj.value) / sp * np.exp(-sp * ss / self.q_circulation / 24.0 / model.reserv.densitywater(self.Tini) / model.reserv.heatcapacitywater(self.Tini) * self.Nonvertical_length.value - sp / self.velocity * self.Nonvertical_length.value)
        return Toutletl
    ...
    def CalculateNonverticalPressureDrop(self, model, time_operation:float, time_max:float, al:float):
        ...
        #------------------------------------------
        NonverticalPressureDrop = [0.0]*model.surfaceplant.plantlifetime.value #intialize the array
        while time_operation <= time_max:
            ...
            Rewaterhoriz = 4.*(self.q_circulation)/(muwater*math.pi*self.nonverticalwellborediameter.value) #assume turbulent flow.
            if self.NonverticalsCased: relroughness = 1E-4/self.nonverticalwellborediameter.value
            else: relroughness = 0.02/self.nonverticalwellborediameter.value   #not the higher relative roughness for uncased nonvertical bores
            ...
            #assume no bouyancy effect becasue laterals are nonvertical, or if they are not, they return to the same place, so there is no bouyancy effect
            NonverticalPressureDrop[year] = friction*(rhowater*vhoriz**2/2)*(self.Nonvertical_length.value/self.nonverticalwellborediameter.value)/1E3      #/1E3 to convert from Pa to kPa
            time_operation += al

        #interpolation is required because NonverticalPressureDrop is sampled yearly, and needs to be sampled more frequently to match other arrays
        f = scipy.interpolate.interp1d(np.arange(0, len(NonverticalPressureDrop)), NonverticalPressureDrop, fill_value="extrapolate")
        NonverticalPressureDrop = f(np.arange(0, len(self.ProducedTemperature.value), 1))   # use interpolation function returned by `interp1d`
    ...
    def Calculate(self, model:AdvModel) -> None:
        ...
        :return: None
        :doc-author: Koenraad Beckers, Malcolm Ross, and Wnaju Yuan
        """
        model.logger.info("Init " + str(__class__) + ": " + sys._getframe().f_code.co_name)
        ...
        if key == None: 
            ...
            if (self.Tini > 375.0) or (self.numnonverticalsections.value > 1): #must be a multilateral setup or too hot for CLGS, so must try to use wanju code.
                if (self.Tini > 375.0):
                    model.logger.warn("In AGS, but forced to use Wanju code becuase initial reservor temperature is too high for CLGS")
                    print ("In AGS, but forced to use Wanju code becuase initial reservor temperature is too high for CLGS") 

                #handle special cases for the multilateral calc parameters you added
                if self.nonverticalwellborediameter.value > 2.0:
                    self.nonverticalwellborediameter.value = self.nonverticalwellborediameter.value * 0.0254   #correct the units of needed
                    self.nonverticalwellborediameter.CurrentUnits = LengthUnit.METERS
                self.area = math.pi * (self.nonverticalwellborediameter.value * 0.5) * (self.nonverticalwellborediameter.value * 0.5)
                self.q_circulation = (self.prodwellflowrate.value / 3.6) / self.numnonverticalsections.value #need to convert prodwellflowrate in l/sec to m3/hour and then split the flow equally across all the sections
                ...
                while self.time_operation.value <= self.time_max:
                    ...
                    self.NonverticalProducedTemperature.value[year] = self.inverselaplace(16, 0, model)
                    self.alpha_fluid = self.WaterThermalConductivity.value / model.reserv.densitywater(self.NonverticalProducedTemperature.value[year]) / model.reserv.heatcapacitywater(self.NonverticalProducedTemperature.value[year]) * 24.0 * 3600.0  #update alpha_fluid value based on next temperature of resevoir
                ...
                #f = scipy.interpolate.interp1d(np.arange(0, len(self.NonverticalProducedTemperature.value)), self.NonverticalProducedTemperature.value)#, fill_value="extrapolate")
                #self.NonverticalProducedTemperature.value = f(np.arange(0, len(model.reserv.Tresoutput.value), 1))   # use interpolation function returned by `interp1d`
                self.NonverticalProducedTemperature.value = signal.resample(self.NonverticalProducedTemperature.value, len(model.reserv.Tresoutput.value))
                ...
                model.reserv.cpwater.value = model.reserv.heatcapacitywater(self.NonverticalProducedTemperature.value[0])
                if self.rameyoptionprod.value: self.ProdTempDrop.value = self.RameyCalc(model.reserv.krock.value, model.reserv.rhorock.value, model.reserv.cprock.value,
                                  self.prodwelldiam.value, model.reserv.timevector.value, model.surfaceplant.utilfactor.value, self.prodwellflowrate.value, 
                                  model.reserv.cpwater.value, model.reserv.Trock.value, model.reserv.Tresoutput.value, model.reserv.averagegradient.value/1000.0, model.reserv.InputDepth.value)

                self.ProducedTemperature.value = self.NonverticalProducedTemperature.value-self.ProdTempDrop.value

                #Now use the parent's calculation to calculate the upgoing and downgoing pressure pressure drops and pumping power
                self.PumpingPower.value = [0.0]*len(self.ProducedTemperature.value) #intialize the array
                if self.productionwellpumping.value:
                    ...
                    self.DPProdWell.value, f3, vprod, self.rhowaterprod = self.WellPressureDrop(model, model.reserv.Tresoutput.value-self.ProdTempDrop.value/4.0, self.prodwellflowrate.value, self.prodwelldiam.value, self.impedancemodelused.value, model.reserv.InputDepth.value)
                    if self.impedancemodelused.value: #assumed everything stays liquid throughout
                        self.DPOverall.value, UpgoingPumpingPower, self.DPProdWell.value, self.DPReserv.value, self.DPBouyancy.value = self.ProdPressureDropsAndPumpingPowerUsingImpedenceModel(f3, vprod,
                                              self.rhowaterinj, self.rhowaterprod, 
                                              self.rhowaterprod, model.reserv.depth.value, self.prodwellflowrate.value, self.prodwelldiam.value, self.impedance.value,
                                              self.nprod.value, model.reserv.waterloss.value, model.surfaceplant.pumpeff.value)
                        self.DPOverall.value, DowngoingPumpingPower, self.DPProdWell.value, self.DPReserv.value, self.DPBouyancy.value = self.ProdPressureDropsAndPumpingPowerUsingImpedenceModel(f3, vprod,
                                              self.rhowaterprod, self.rhowaterinj, model.reserv.rhowater.value, model.reserv.depth.value, self.prodwellflowrate.value, self.injwelldiam.value, self.impedance.value,
                                              self.nprod.value, model.reserv.waterloss.value, model.surfaceplant.pumpeff.value)

                    else: #PI is used for both the verticals
                        UpgoingPumpingPower, self.PumpingPowerProd.value, self.DPProdWell.value, self.Pprodwellhead.value =  self.ProdPressureDropAndPumpingPowerUsingIndexes(model, self.usebuiltinhydrostaticpressurecorrelation, self.productionwellpumping.value, self.usebuiltinppwellheadcorrelation,
                        ...
                        DowngoingPumpingPower, ppp2, dppw, ppwh =  self.ProdPressureDropAndPumpingPowerUsingIndexes(model, self.usebuiltinhydrostaticpressurecorrelation, self.productionwellpumping.value, self.usebuiltinppwellheadcorrelation,
                                                    model.reserv.Trock.value, model.reserv.Tsurf.value, model.reserv.depth.value, model.reserv.averagegradient.value, self.ppwellhead.value, self.PI.value, self.prodwellflowrate.value, f3, vprod,
                                                    self.injwelldiam.value, self.nprod.value, model.surfaceplant.pumpeff.value, self.rhowaterinj)
                
                    #Calculate Nonvertical Pressure Drop
                    NonverticalPumpingPower = [0.0]*len(DowngoingPumpingPower) #intialize the array
                    self.NonverticalPressureDrop.value, f3 = self.CalculateNonverticalPressureDrop(model, self.time_operation.value, self.time_max, self.al)

                    #calculate nonvertical well pumping power needed[MWe]
                    NonverticalPumpingPower = self.NonverticalPressureDrop.value*self.nprod.value*self.prodwellflowrate.value/self.rhowaterprod/model.surfaceplant.pumpeff.value/1E3 #[MWe] total pumping power for nonvertical section
            ...
            else:   #do the CLSG-style calculation
                ...
                # getTandP results must be rejiggered to match wellbores expected output. Once done, the surfaceplant and economics models should just work            #overall pressure drop  = previous pressure drop (as calculated from the verticals) + nonvertical section pressure drop
                #interpolation is required because DPOverall is sampled slightly differently, and DPOverall is sampled more frequently
                f = scipy.interpolate.interp1d(np.arange(0, len(self.DPOverall.value)), self.DPOverall.value, fill_value="extrapolate")
                self.DPOverall.value = f(np.arange(0, len(self.DPOverall.value), 1))   # use interpolation function returned by `interp1d`
